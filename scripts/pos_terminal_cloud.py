...
with st.sidebar:
    ...
    st.info("Operating in Cloud Pub/Sub Mode")
    project_id = st.text_input("GCP Project ID", value=os.environ.get("GCP_PROJECT_ID", "YOUR-PROJECT-ID"))
...
def get_pubsub_clients(proj_id):
    ...
    resolution_sub_path = subscriber.subscription_path(proj_id, "fraud-resolutions-sub")
    return publisher, topic_path, subscriber, alert_sub_path, resolution_sub_path

try:
    publisher, topic_path, subscriber, alert_sub_path, resolution_sub_path = get_pubsub_clients(project_id)
except Exception as e:
    st.error(f"Failed to connect to GCP Pub/Sub: {e}")
    st.stop()
...
while True:
    ...
    try:
        alert_response = subscriber.pull(request={"subscription": alert_sub_path, "max_messages": 10}, timeout=0.2)
        if alert_response.received_messages:
            ui_needs_update = True
        for msg in alert_response.received_messages:
            ...
            subscriber.acknowledge(request={"subscription": alert_sub_path, "ack_ids": [msg.ack_id]})
    ...
    try:
        res_response = subscriber.pull(request={"subscription": resolution_sub_path, "max_messages": 10}, timeout=0.2)
        if res_response.received_messages:
            ui_needs_update = True
        for msg in res_response.received_messages:
            res = json.loads(msg.message.data.decode('utf-8'))
